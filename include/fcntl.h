...
// 下面是文件创建和操作 ,用于open().可与上面访问模式用'位或'的方式一起使用.
...
// 下面是文件锁定命令。fnctl()的第三个参数lock是指向flock结构的指针。
...
/* 同样 - 也还没有实现，但是... */
// 文件锁定操作数据结构。描述了受影响文件段的类型（l_type）、开始偏移（l_whence）、相对偏移（l_start）、
// 锁定长度（l_len）和实施锁定的进程id。
struct flock {
...
// 文件句柄操作，会影响文件的打开。
// 参数fildes是文件句柄，cmd是操作命令。该函数可有以下几种形式：
