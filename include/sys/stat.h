...
// 下面是为st_mode字段所用的值定义的符号名称.这些值均用八进制表示.为便于记忆,这些符号名称均为一些英文单词的首字母或缩写组合而成.例如名称S_IFMT的每
// 个字母分别代表单词State,Inode,File,Mask和Type;而名称s_IFREG则是State,Inode,File和REGular几个大写字母的组合;名称S_IRWXU是State,Inode,Read,Write,
// Execute和User的组合.其他名称可以此类推.
// 文件类型:
...
// 文件属性位:
// S_ISUID用于测试文件的set-user-ID标志是否置位.若该标志置位,则当执行该文件时,进程的有效用户ID将被设置为该文件宿主的用户ID.S_ISGID则是针对组ID进行相同处理.
