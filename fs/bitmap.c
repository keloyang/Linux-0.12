...
// 第20行定义了一个局部寄存器变量res.该变量将被保存在指定的eax寄存器中,以便于高效访问和操作.这种定义变量的方
// 法主要用于内嵌汇编程序中.整个宏定义是一个语句表达式,该表达式值是最后res的值.第21行上的btsl指令用于测试并
// 设置位(Bit Test and Set).把基地址(%3)和位偏移值(%2)所指定的位值先保存到进位标志CF中,然后设置该位为1.
...
// 输入：%0 -eax（返回值）；%1 -eax（0）；%2 -nr，位偏移值；%3 -（addr），addr的内容。btrl指令用与测试并
// 复位位（Bit Test and Reset）。其作用与上面的btsl类似，但是复位指定位。指令setnb用于根据进位标志CF设置
// 操作数（%al）。如果CF = 1则%al = 0,否则%al = 1。
...
// 在addr指定地址开始的位图中寻找第1个是0的位,并将其距离addr的位偏移值返回.addr是缓冲块数据区的地址,扫描寻找
// 的范围是1024字节(8192位).
