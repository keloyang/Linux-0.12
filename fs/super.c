...
// 测试指定位偏移处位的值,并返回该原位值(应该取名为test_bit()更妥帖).
// 嵌入式汇编宏.参数bitnr是位偏移值,addr是测试位操作的起始地址.
// %0 - ax(__res),%1 - 0,%2 - bitnr,%3 - addr
// 第23行定义了一个局部寄存器变量.该变量将被保存在eax寄存器中,以便于高效访问和操作.第24行上指令bt用于对位进行测试(Bit Test).它会把地址addr(%3)和
// 位偏移量bitnr(%2)指定的位的值放入进位标志CF中.指令setb用于根据进位标志CF设置操作数%al.如果CF = 1则%al = 1,否则%al = 0.
