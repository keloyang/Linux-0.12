 ...
 */
/*
 * 需求加载是从91.12.1开始编写的确 - 在程序编制表中似乎是最重要的程序，并且应该是很容易编制的 - Linus
 */
 ...
 */
/*
 * Ok，需求加载是比较容易编写的，而共享页面却需要有点技巧。共享页面程序是91.12.2开始编写的，好像能够工作 - Linus。
 *
 * 通过执行大约30个/bin/sh对共享操作进行了测试：在老内核当中需要占用多于6MB的内在，而目前却不用。现在
 * 看来工作得很好。
 *
 ...
 */
/*
 * 91.12.18开始编写真正的虚拟内存管理VB（交换页面到/从磁盘）。需要对此考虑很多并且需要作很多工作。呵呵，也只能这样了。
...
// 该宏用于判断给定线性地址是否位于当前进程的代码段中，“(((addr)+4095)&~4095)”用于取得线性地址addr所在内存页面的
// 末端地址。
 ...
 */
/*
 * 下面函数释放页表连续的内存块,exit()需要该函数.与copy_page_tables()类似,该函数仅处理4MB长度的内存块.
 */
 ...
 */
/*
 * 好了,下面是内存管理mm中最为复杂的程序之一.它通过只复制内存页面来复制一定范围内线性地址中的内容.希望代码中没有错误,因为我不想
 * 再调试这块代码了:-)
 *
 * 注意!我们并不复制任何内存块,内存块的地址需要是4MB的倍数(正好一个页目录项对应的内存长度),因为这样处理可使函数简单.不管怎样,它仅被
 * fork()使用.
 *
 * 注意2!!当from==0时,说明是在为第一次fork()调用复制内核空间.此时我们就不想复制整个页目录项对应的内存,因为这样做会导致内存严重浪费
 * 我们只须复制开头160个页面,对应640KB.即使是复制这些页面也已经超出我们的需求,但这不会占用更多的内存,在低1MB内存范围内不执行写时复制
 * 操作,所以这些页面可以与内核共享.因此这是nr=xxxx的特殊情况(nr在程序中指页面数).
 */
// 复制目录表项和页表项.
	...
	for( ; size-- > 0 ; from_dir++, to_dir++) {
		...
		*to_dir = ((unsigned long) to_page_table) | 7;
		...
		for ( ; nr-- > 0 ; from_page_table++, to_page_table++) {
			...
			this_page &= ~2;
			*to_page_table = this_page;
			// 如果该页表项所指物理页面的地址在1MB以上,则需要设置内存页面映射数组mem_map[],于是计算页面号,并以它为索引在页面同数组相应项中增加引用次数.而对于位于1MB以下
 ...
 */
/*
 * 下面函数将一内存页面放置(映射)到指定线性地址处.它返回页面的物理地址,如果内存不够(在访问页面或页面时),则返回0.
 */
 ...
 */
/*
 * 如果你也想设置页面已修改标志,则上一个函数工作得不是很好:exec.c程序需要这种设置.因为exec.c中函数会在放置页面之前修改过页面内容.为了实现VM,我们需要能正确设置
 ...
 */
/*
 * 当用户试图住一共享页面上写时,该函数处理已存在的内存页面(写时复制),它是通过将页面复制到一个新地址上并且递减原页面的共享
 * 计数值实现的.
 *
...
void do_wp_page(unsigned long error_code, unsigned long address)
{
	...
	if (CODE_SPACE(address))
		do_exit(SIGSEGV);
#endif
	// 调用上面函数un_wp_page()来处理取消页面保护.但首先需要为其准备好参数. 参数是线性地址address指定页面在页表中的页表项指针,其计算方法是:1
	// ((address>>10) & 0xffc):计算指定线性地址中页表项在页表中的偏移地址;因为根据线性地址结构,(address >> 12)就是页表项中索引,但每项占4
	// 个字节,因此乘4后:(address>>12)<<2=(address>>10)&0xffc就可得到页表项在表中的偏移地址.与操作&0xffc用于限制地址范围在一个页面内.又因为
	// 只移动了10位,因此最后2位是线性地址低12位中的最高2位,也应屏蔽掉.因此求线性地址中页表项在页表中偏移地址直观一些的表示方法是(((address>>12)&0x3ff)<<2)
	// 2:(0xfffff000&*((address>>20)&0xffc)):用于取目录项中页表的地址值;其中,((address>>20)&0xffc)用于取线性地址中的目录索引项在目录表中的偏移
...
// 当前进程与进程p是同一执行代码，也可以认为当前进程是由p进程执行fork操作产生的进程，因此它们的代码内容一样。如果未对数据
...
static int try_to_share(unsigned long address, struct task_struct * p)
{
	...
	// 在得到p进程和当前进程address对应的目录项后，下面分别对进程p和当前进程处理。首先对p进程的表项进行操作。目录是取得p进程中
 ...
 */
/*
 * share_page()试图找到一个进程,它可以与当前进程共享页面.参数address是当前进程数据空间中期望共享的某页面地址.
 *
 * 首先我们通过检测executable->i_count来查证是否可行.如果有其他任务已共享该inode,则它应该大于1.
 */
// 共享页面处理.
// 在发生缺页异常时,首先看看能否与运行同一个执行文件的其他进程页面共享处理.该函数首先判断系统中是否有另一个进程也在运行与当前进程
// 一样的执行文件.若有,则在系统当前所有任务中寻找这样的任务.若找到了这样的任务就尝试与其共享指定地址处的页面.若系统中没有其他任务
// 正在运行与当前进程相同的执行文件,那么共享页面操作的前提条件不存在,因此函数立刻退出.判断系统中是否有另一个进程也在执行同一个执
// 行文件的方法是利用进程任务数据结构中的executable字段(或library字段).该字段指向进程正在执行程序(或使用的库文件)在内存中的i节
...
static int share_page(struct m_inode * inode, unsigned long address)
{
	...
	if (inode->i_count < 2 || !inode)
		return 0;
	// 否则搜索任务数组中所有任务.寻找与当前进程可共享页面的进程,即运行相同执行文件的另一个进程,并尝试对指定地址的页面进行共享.若进程逻
