 ...
 * head.s含有32位启动代码。
 *
 * 注意!!! 32位启动代码是从绝对地址0x00000000开始的,这里也同样是页目录将存在的地方,因此这里的启动代码将被页目录覆盖掉.
 *
 */
.text
...
pg_dir:# 页目录将会存放在这里.
 ...
 # 地说是一个描述符表项的选择符.这里$0x10的含义是请求特权级0(位0-1=0),选择全局描述符表(位2=0),选择表中第2项(位3-15=2).它正好指向表中的
 # 数据段描述符项.
 # 下面的代码含义是:设置ds,es,fs,gs为setup.s中构造的数据段(全局段描述符表第2项)的选择符=0x10,并将堆栈放置在stack_start指向的user_stack
 # 数据区,然后使用本程序后面定义的新中断描述符表和全局段描述符表.新全局段描述表中初始内容与setup.s中的基本一样,仅段限长从8MB修改成了16MB.
 # stack_start定义在kernel/sched.c中.它是指向user_stack数组末端的一个长指针.下面设置这里使用的栈,姑且称为系统栈.但在移动到任务0执行(
 # init/main.c中)以后该栈就被用作任务0和任务1共同使用的用户栈了.
.globl startup_32
startup_32:
 ...
 */
/*
 * 注意!在下面这段程序中,486应该将们16置位,以检查在超级用户模式下的写保护,此后"verify_area()"调用就不需要了.486的用户通常也会想将NE(#5)置位,以便对数学
 * 协处理器的出错使用int 16.
 *
 */
 # 上面原注释中提到的486CPU中CR0控制器的位16是写保护标志WP,用于禁止超级用户级的程序向一般用户只读页面中进行写操作.该标志主要用于操作系统在创建新进程时实现写
 # 里复制方法.
 # 下面这段程序用于检查数学协处理器芯片是否存在.方法是修改控制寄存器CR0,在假设存在协处理器的情况下执行一个协处理器指令,如果出错的话则说明协处理器芯片不存在,需要
 ...
 *
 * 将中断描述符表idt设置成具有256个项,并都指向ignore_int中断门.然后加载中断描述符表寄存器(lidt指令).真正实用的中断门以后再安装.当我们在其他地方认为一切都正常时再开启
 * 中断.该子程序将会被页表覆盖掉.
 */
